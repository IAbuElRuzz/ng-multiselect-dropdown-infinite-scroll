...
import { ListItem, IDropdownSettings } from "./multiselect-infinite-scroll.model";
import { ListFilterPipe } from "./list-filter.pipe";

export const DROPDOWN_CONTROL_VALUE_ACCESSOR: any = {
  provide: NG_VALUE_ACCESSOR,
  useExisting: forwardRef(() => MultiSelectInfiniteScrollComponent),
  multi: true
};
const noop = () => {};
...
@Component({
  ...
  selector: "ng-multiselect-dropdown-infinite-scroll",
  templateUrl: "./multi-select-infinite-scroll.component.html",
  styleUrls: ["./multi-select-infinite-scroll.component.scss"],
  providers: [DROPDOWN_CONTROL_VALUE_ACCESSOR],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class MultiSelectInfiniteScrollComponent implements ControlValueAccessor {
  ...
  @Output("onScrollEnd")
  onScrollEnd: EventEmitter<boolean> = new EventEmitter<boolean>();
  ...
  onScrollEndCompleted($event: Event) {
    const inputSelect = $event.target as HTMLInputElement;
    if((inputSelect.scrollTop + inputSelect.offsetHeight + 10) > inputSelect.scrollHeight){
         this.onScrollEnd.emit(true);
    }else{
        this.onScrollEnd.emit(false);
    }   
  }
